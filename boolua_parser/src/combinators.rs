...
use boolua_lexer::{string, Token, T};
...
pub fn chunk(source: &str) -> impl Parser<Token, Block, Error = Simple<Token>> + '_ {
    ...
    let numlit = choice((
        J![declit].map_with_span(|_, span| crate::number::eval_num_lit::<10>(&source[span])),
        J![hexlit].map_with_span(|_, span| crate::number::eval_num_lit::<16>(&source[span])),
    ));
